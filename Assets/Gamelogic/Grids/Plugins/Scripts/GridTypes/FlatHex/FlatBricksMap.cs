...
namespace Gamelogic.Grids
{
	...
	public class FlatBrickMap : AbstractMap<FlatHexPoint>
	{
		...
		{
			int x = Mathf.FloorToInt((point.x + cellDimensions.x / 2) / cellDimensions.x);
			int y = Mathf.FloorToInt((point.y - x * cellDimensions.y / 2 + cellDimensions.y / 2) / cellDimensions.y);
